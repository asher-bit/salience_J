...
# 对张量划分窗口，指定窗口大小
# 原始张量 B H W C ---> num_windows*B, window_size, window_size, C, num_windows是窗口个数
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # numWindows*B, num_heads, window_size*window_size, window_size*window_size

        # 得到相对位置索引，选取编码，加到attn张量上
        ...
        # shift操作
        # 通过mask操作，保证widnwos数量不变的情况下实现操作
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N, N)
            # mask对应位置的值为-100，经过softmax就变成了0
...
# 在每个stage前做降采样，用来缩小分辨率，调整通道数
# 在行和列上间隔2选取元素
# 拼接在一起作为一个tensor，然后展开，通道数变为之前的4倍
# 再通过全连接调整通道数为2倍
...
class Salicon(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.conv1 = nn.Conv2d(9, embed_dim//3, 3, 1, 1)
        self.conv2 = nn.Conv2d(9, embed_dim//3, 3, 1, 1)
        self.conv3 = nn.Conv2d(9, embed_dim//3, 3, 1, 1)
    ...
    def forward(self, raw_img, img_me1, img_me2, img_me3, img_me4, img_me5, img_me6, img_me7, img_me8, img_me9):
        ...
        x = self.conv_after_body(self.forward_features(x)) + self.conv_end(x_2)
